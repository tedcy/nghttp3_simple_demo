...
public:
    ...
    */
    ...
    static time_t str2time_t(const string &sString, const string &sFormat);
    ...
    */
    template <typename T>
    static typename std::enable_if<!IsSpecifyFormatType<T>::value, T>::type strto(const string &sStr);
    ...
    */
    ...
    static string join(Container const& c, string const& sSep);
    ...
    * @return  转换后的字符串
    */
    static string tostr(const bool &t);
    static string tostr(const char &t);
    static string tostr(const unsigned char &t);
    static string tostr(const short &t);
    static string tostr(const unsigned short &t);
    static string tostr(const int &t);
    static string tostr(const unsigned int &t);
    static string tostr(const long &t);
    static string tostr(const long long &t);
    static string tostr(const unsigned long &t);
    static string tostr(const float &t);
    static string tostr(const double &t);
    static string tostr(const long double &t);
    static string tostr(const std::string &t);

    template <typename T>
    struct IsNormalToStrType
    {
        static const bool value =
            std::is_same<T, bool>::value ||
            std::is_same<T, char>::value ||
    ...
    template<typename T>
    static typename std::enable_if<!std::is_base_of<JceStructBase, T>::value, string>::type
    tostr(const T &t)
    {
        if (IsNormalToStrType<T>::value) {
            return tostr(t);
        }
    ...
    /**
     * @brief  打印字段名且不换行输出JCE结构体
     *
     * @param t JCE结构体
     * @return  转换后的字符串
     */
    template<typename T>
    static string tostr_nobr(const T &t)
    {
        string Originstr = tostr(t);
        size_t slen = Originstr.length(), index = 0;
        bool pre_is_nt = false;
        bool pre_is_open_bracket = false;
        bool next_is_close_bracket = false;
        string ret;
        ret.reserve(slen);
        for(index = 0; index < slen; ++index)
        {
            ...
            {
                //连续\n\t 或者 前一个是开
                if(pre_is_nt || pre_is_open_bracket)
                    continue;
                size_t nindex = index + 1;
                next_is_close_bracket = true;
                while(nindex < slen)
                {
                    if(Originstr[nindex] == '\n' || Originstr[nindex] == '\t')
                        ++nindex;
                    else if(Originstr[nindex] == '}' || Originstr[nindex] == ')' || Originstr[nindex] == ']')
                        break;
                    else{
                        next_is_close_bracket = false;
                        break;
                    }
                }
                //后一个是闭
                if(next_is_close_bracket)
                    continue;
                pre_is_nt = true;
                pre_is_open_bracket = false;
                ret += ", ";
            }
            else{
                char ch = Originstr[index];
                ret += ch;
                pre_is_nt = false;
                pre_is_open_bracket = false;
                if(ch == '{' || ch == '(' || ch == '[')
                    pre_is_open_bracket = true;
            }
        }
        return ret;
    }
    ...
    *     TC_Common::tostr(s.begin(), s.end(), [](auto &kv) {
    *         return kv.first + ":" + kv.second;
    *     });
    */
    template <typename F, typename Input, typename = void>
    struct tostr_callable : std::false_type {};

    template <typename F, typename Input>
    struct tostr_callable<
        F, Input, void_t_cpp11<decltype(declval<F>()(declval<Input &>()))>>
        : std::true_type {};

    template <
        typename InputIter,
        typename Input =
            std::remove_reference_t<decltype(*declval<InputIter>())>,
        typename F = function<string(Input &)>,
        typename enable_if<tostr_callable<F, Input>::value>::type * = nullptr>
    static string tostr(InputIter iFirst, InputIter iLast, F &&f,
                        const string &sSep = "|");
    ...
    */
    ...
    static string waitResult2Str(int status);
    ...
    static vector<char> str2vec(string const& s) {
        vector<char> vec(s.size());
        ::memcpy(&vec[0], &s[0], s.size());
        return vec;
    }

    static string vec2str(vector<char> const& vec) {
        string s;
        s.resize(vec.size());
        ::memcpy(&s[0], &vec[0], vec.size());
        return s;
    }
};
...
template <typename Container>
string TC_Common::join(Container const& c, string const& sSep)
{
    string s;
...
template<typename T>
string TC_Common::tostr(const set<T> &t)
{
    string s;
    for (T const& v : t)
    {
...
template <
    typename InputIter, typename Input, typename F,
    typename enable_if<TC_Common::tostr_callable<F, Input>::value>::type *>
string TC_Common::tostr(InputIter iFirst, InputIter iLast, F &&f,
                        const string &sSep) {
    string sBuffer;
    InputIter it = iFirst;
...
template <typename T>
typename std::enable_if<!IsSpecifyFormatType<T>::value, T>::type TC_Common::strto(const string &sStr)
{
...
typename std::enable_if<IsSpecifyFormatType<T>::value, T>::type TC_Common::strto(const string &sStr)
{
    return static_cast<T>(strto<int>(sStr));
}
