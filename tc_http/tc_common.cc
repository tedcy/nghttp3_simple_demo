...
string TC_Common::tostr(const bool &t)
{
    char buf[2];
    buf[0] = t ? '1' : '0';
    buf[1] = '\0';
...
{
    char buf[16];
    snprintf(buf, sizeof(buf), "%d", static_cast<int>(t));
    return string(buf);
}

string TC_Common::tostr(const unsigned char &t)
{
    char buf[16];
...
string TC_Common::tostr(const short &t)
{
    char buf[16];
    snprintf(buf, 16, "%d", t);
    return string(buf);
...
string TC_Common::tostr(const unsigned short &t)
{
    char buf[16];
    snprintf(buf, 16, "%u", t);
    return string(buf);
...
string TC_Common::tostr(const int &t)
{
    char buf[16];
    snprintf(buf, 16, "%d", t);
    return string(buf);
...
string TC_Common::tostr(const unsigned int &t)
{
    char buf[16];
    snprintf(buf, 16, "%u", t);
    return string(buf);
...
string TC_Common::tostr(const long &t)
{
    char buf[32];
...
string TC_Common::tostr(const long long &t)
{
    char buf[32];
...
string TC_Common::tostr(const unsigned long &t)
{
    char buf[32];
...
string TC_Common::tostr(const float &t)
{
    char buf[32];
...
string TC_Common::tostr(const double &t)
{
    char buf[32];
    snprintf(buf, 32, "%.5f", t);
    string s(buf);
...
string TC_Common::tostr(const long double &t)
{
    char buf[32];
...
string TC_Common::tostr(const std::string &t)
{
    return t;
}
...
time_t TC_Common::str2time_t(const string &sString, const string &sFormat)
{
    struct tm t;
    memset(&t, 0, sizeof(t));
    int res = str2tm(sString, sFormat, t);
    if (res == -1)
        return 0;
...
int TC_Common::str2bin(const char *psAsciiData, unsigned char *sBinData, int iBinSize)
{
    ...
    {
        sBinData[i] = x2c(*(psAsciiData + i*2), *(psAsciiData + i*2 + 1));
    }
    return iRealLength;
}
...
string TC_Common::str2bin(const string &sString, const string &sSep, size_t lines)
{
    ...
    {
        sBinData += x2c(*(psAsciiData + i), *(psAsciiData + i + 1));
        i++;
...
char TC_Common::x2c(char a, char b)
{
    char digit;

    digit = (a >= 'A' ? ((a & 0xdf) - 'A')+10 : (a - '0'));
    digit *= 16;
    digit += (b >= 'A' ? ((b & 0xdf) - 'A')+10 : (b - '0'));
...
string TC_Common::waitResult2Str(int status) {
    std::stringstream ss;
    if (WIFEXITED(status)) {
        ss << "Child exited normally with status " << WEXITSTATUS(status)
           << std::endl;
    } else if (WIFSIGNALED(status)) {
        ss << "Child terminated by signal " << WTERMSIG(status);

        if (WCOREDUMP(status)) {
            ss << " and Core dump was produced";
        }

        ss << std::endl;
    } else {
        ss << "Child ended in an unexpected manner" << std::endl;
    }
    return ss.str();
}
