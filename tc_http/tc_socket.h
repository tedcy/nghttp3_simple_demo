...
public:
     ...
     *               AF_LOCAL：
     */
    int getDomain() const { return _iDomain; }
    ...
    * @return
    */
    static void parseAddr(int domain, const string &sAddr, struct sockaddr_storage & stAddr);
    static void parseAddr(const string &sAddr, struct in_addr &stSinAddr);

    static pair<string, uint32_t> sockaddr2IpPort(
        const struct sockaddr_in *addr);
    // 将 IP 和端口转换为 sockaddr_in 结构体
    static struct sockaddr_in ipPort2Sockaddr(const std::string &ip, uint32_t port);

    // 解析地址
    // 如果sHost是ip地址, 直接返回
    // 如果sHost是一个域名, 解析成ip地址列表返回
    static vector<string> host2Addrs(const string &sHost, string * err = nullptr);

    static int getEstSize(int bytes) {
        const int mss = 1500 - 20 - 20;
        int recvPacketNum = bytes / mss + (bytes % mss == 0 ? 0 : 1);
        return bytes + recvPacketNum * (20 + 20);
    }
...
public:
    // 设置可以多进程共用的端口号
    // @ 注意要在Application::main之前设置.
    static void addReusePort(int port);
