...
size_t TC_HttpResponse::getContentLength() const
{
    const_cast<TC_HttpResponse&>(*this).mergerChunked();
    return TC_Http::getContentLength();
}

string TC_HttpResponse::getContent() const 
{
    const_cast<TC_HttpResponse&>(*this).mergerChunked();
    return TC_Http::getContent();
}

void TC_HttpResponse::mergerChunked()
{
    //std::cout << _listContent.size() << std::endl;
    _content.reserve(_bodySize);
    auto l = std::move(_listContent);
...
bool TC_HttpResponse::incrementDecode(string &sBuffer, int requestType)
{
    //解析头部
    if(_headLength == 0)
    {
    ...
    // HEAD请求没有body
    if (requestType == TC_HttpRequest::REQUEST_HEAD) {
        return true;
    }
    ...
    {
        int readIdx = 0;
        std::shared_ptr<int> _atExit(nullptr, [&](int *) {
            if(readIdx > 0) {
                if(readIdx < int(sBuffer.size())) {
                    sBuffer = sBuffer.substr(readIdx);
                } else {
                    sBuffer.clear();
                }
            }
        ...
        while(true)
        {
            ...
            //查找当前chunk的大小
            string sChunkSize       = sBuffer.substr(readIdx, pos - readIdx);
            ...
            if(sBuffer.length() >= pos + 2 + (size_t)iChunkSize + 2)   //接收到一个完整的chunk了
            {
                ...
                _listContent.push_back(sBuffer.substr(pos + 2, iChunkSize));
                _bodySize += iChunkSize;
                readIdx = pos + 2 + iChunkSize + 2;
            }
            else
            {
                //没有接收完整的chunk
...
vector<string> TC_HttpRequest::getCookie()
{
    ...
    for( http_header_type::const_iterator it = _headers.begin(); it != itEnd; ++it)
    {
        if(TC_Common::lower(it->first) == "cookie")
        {
            v.push_back(it->second);
        }
    }
